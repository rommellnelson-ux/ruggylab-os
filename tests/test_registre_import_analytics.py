...
SAMPLE_ROWS = [
    {"nom": "M. ESSOH Paul", "date": "13/03/2025", "type_registre": "Hors CMU",
     "examens": "NFS ; Urée 0,13 ; Créat 77,2", "montant": 7000, "part_cmu": 0,
     "prescripteur": "Wognin", "provenance": "G.R"},
    {"nom": "KOUADJO Ahou Ruth", "date": "20/03/2025", "type_registre": "Hors CMU",
     "examens": "GE +145 trophozoïtes/champ", "montant": 2000, "part_cmu": 0,
     "prescripteur": "Wognin", "provenance": "G.R", "age": "32 ans"},
    {"nom": "KADJO Maxime", "date": "20/03/2025", "type_registre": "CMU",
     "examens": "CRP ; NFS ; GE négative", "montant": 7000, "part_cmu": 4900,
     "prescripteur": "Dr Wognin", "age": "46 ans"},
]
...
class TestRegistreImport:
    def test_dry_run_counts_without_writing(self, client):
        ...
        r = client.post("/api/v1/registre/import", headers=hdrs,
                        json={"rows": SAMPLE_ROWS, "dry_run": True})
        assert r.status_code == 200, r.text
        body = r.json()
    ...
    def test_import_is_audited(self, client):
        hdrs = _auth(client)
        client.post("/api/v1/registre/import", headers=hdrs,
                    json={"rows": SAMPLE_ROWS, "dry_run": False, "confirm": True})
    ...
    def test_import_requires_officer(self, client):
        tech = _tech(client)
        if tech:
            r = client.post("/api/v1/registre/import", headers=tech,
                            json={"rows": SAMPLE_ROWS, "dry_run": True})
