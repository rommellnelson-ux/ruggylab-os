...
@router.get("", response_model=EquipmentReagentRatioListResponse, dependencies=[Depends(require_admin)])
def list_ratios(
    ...
    return EquipmentReagentRatioListResponse(items=items, meta=PaginationMeta(total=total, skip=skip, limit=limit))


@router.get("/{ratio_id}/versions", response_model=list[EquipmentReagentRatioVersionRead], dependencies=[Depends(require_admin)])
def list_ratio_versions(ratio_id: int, db: Session = Depends(get_db)) -> list[EquipmentReagentRatioVersionRead]:
...
@router.post("", response_model=EquipmentReagentRatioRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_admin)])
def create_ratio(
    ...
    if not equipment or not reagent:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Equipement ou reactif introuvable.")
    existing = (
        db.query(EquipmentReagentRatio)
        .filter(EquipmentReagentRatio.equipment_id == payload.equipment_id, EquipmentReagentRatio.reagent_id == payload.reagent_id)
        .first()
    )
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Ratio deja existant pour ce couple.")
    ratio = EquipmentReagentRatio(**payload.model_dump())
    db.add(ratio)
    db.flush()
    create_ratio_version(db, ratio=ratio, changed_by_user=current_user, change_reason="Initial creation")
    log_audit_event(db, user=current_user, event_type="equipment_reagent_ratio.create", entity_type="equipment_reagent_ratio", entity_id=str(ratio.id), payload=payload.model_dump())
    db.commit()
    db.refresh(ratio)
    return ratio


@router.put("/{ratio_id}", response_model=EquipmentReagentRatioRead, dependencies=[Depends(require_admin)])
def update_ratio(
    ratio_id: int,
    ...
    create_ratio_version(db, ratio=ratio, changed_by_user=current_user, change_reason="Admin update")
    log_audit_event(db, user=current_user, event_type="equipment_reagent_ratio.update", entity_type="equipment_reagent_ratio", entity_id=str(ratio.id), payload=payload.model_dump(exclude_none=True))
    db.commit()
    db.refresh(ratio)
    return ratio
...
def delete_ratio(
    ...
    if not ratio:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Ratio introuvable.")
    log_audit_event(db, user=current_user, event_type="equipment_reagent_ratio.delete", entity_type="equipment_reagent_ratio", entity_id=str(ratio.id), payload={"equipment_id": ratio.equipment_id, "reagent_id": ratio.reagent_id})
    db.delete(ratio)
    db.commit()
