...
| ACQ-OPEN-01 | P1 intégrité | `results_poct.submit_poct_batch` ligne 160 et `results_qualitative.submit_qualitative_result` ligne 36 ajoutent puis committent un `Result` sans clé d'acquisition. | Rejeu réseau : doublon et possible double consommation ; déduplication naïve : suppression d'une mesure légitime. | Transaction locale ; statut annulé refusé ; stock atomique. | Deux appels identiques séquentiels et concurrents avec futur identifiant d'acquisition. | Contrat métier absent. Décision D2. **Escaladé**. |
...
| IMG-OPEN-01 | P1 clinique | `malaria_ai._heuristic_predict` vers 207 est marqué non clinique ; `process_malaria_job` positionne `Result.is_critical` vers 297. | Un nom/chemin d'image influence un résultat et un drapeau critique sans modèle validé. | Marqueur `real_inference`, audit, modèle réel préféré si chargé. | Modèle absent : aucune mutation clinique autorisée en profil production. | Décision clinique/exploitation D4. **Escaladé**. |
...
| CI-OPEN-01 | P3 exploitation | `.github/workflows/ci.yml` utilise `setup-node` vers 225 et `upload-artifact` vers 174/255 ; annotation CI Node 20→24. | Une future suppression de compatibilité casse E2E/artifacts. | SHA des actions épinglés ; runner force actuellement Node 24. | CI après montée des actions. | Mettre à niveau dans un lot CI dédié. **Ouvert**. |
