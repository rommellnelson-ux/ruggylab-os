...
| EQUIP-REG-01 | P1 clinique, intégrité, sécurité | Le modèle minimal ne pouvait pas démontrer l'identité technique, le périmètre analytique ni la qualification d'une interface. | Migration additive `20260724_0039`, registre normalisé, snapshots versionnés, RBAC EQUIP-RBAC-01, activation centralisée fail-closed et contrôles à l'usage. | Les interfaces restent désactivées sans qualification complète et correspondante ; aucun équipement réel n'est qualifié par le code. **Corrigé techniquement ; qualification réelle toujours requise**. |
